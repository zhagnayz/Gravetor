...
//  GravetonDriver
//
//  Created by Daniel Zhagany Zamora on 3/15/23.
//
...
final class LocationManager: NSObject {
    ...
    typealias ReverseGeoLocationClosure = ((_ location:CLLocation?,_ placemark: CLPlacemark?,_ error:NSError?) ->())
    ...
    @objc private func sendPlacemark() {
        ...
        guard let _ = lastLocation else {
            ...
            self.didCompleteGeocoding(location: nil, placemark: nil, error: NSError(domain: self.classForCoder.description(), code:Int(CLAuthorizationStatus.denied.rawValue),userInfo:[NSLocalizedDescriptionKey:LocationErrors.notFetched.rawValue,
    ...
    @objc private func sendLocation() {
        ...
        guard let _ = lastLocation else {
            ...
            self.didComplete(location: nil,error: NSError(domain: self.classForCoder.description(),
                code:Int(CLAuthorizationStatus.denied.rawValue),
                userInfo:[NSLocalizedDescriptionKey:LocationErrors.notFetched.rawValue,
              NSLocalizedFailureReasonErrorKey:LocationErrors.notFetched.rawValue,
         NSLocalizedRecoverySuggestionErrorKey:LocationErrors.notFetched.rawValue]))
            lastLocation = nil
            return
    ...
    private func reverseGeoCoding(location:CLLocation?) {
        ...
        CLGeocoder().reverseGeocodeLocation(location!, completionHandler: {(placemarks, error)->Void in
            ...
            if (error != nil) {
                ...
                self.didCompleteGeocoding(location: nil, placemark: nil, error: NSError(domain: self.classForCoder.description(),
                    code:Int(CLAuthorizationStatus.denied.rawValue),
            ...
            if placemarks!.count > 0 {
                ...
                if let _ = location {
                    ...
                    self.didCompleteGeocoding(location: nil, placemark: nil, error: NSError(
                        ...
                        code:Int(CLAuthorizationStatus.denied.rawValue),
                        userInfo:[NSLocalizedDescriptionKey:LocationErrors.invalidLocation.rawValue,
    ...
    private func reverseGeoCoding(address:String) {
        ...
        CLGeocoder().geocodeAddressString(address, completionHandler: {(placemarks, error)->Void in
            if (error != nil) {
                //Reverse geocoding failed
                self.didCompleteGeocoding(location: nil, placemark: nil, error: NSError(domain: self.classForCoder.description(),
                    code:Int(CLAuthorizationStatus.denied.rawValue),userInfo:[NSLocalizedDescriptionKey:LocationErrors.reverseGeocodingFailed.rawValue,
                  NSLocalizedFailureReasonErrorKey:LocationErrors.reverseGeocodingFailed.rawValue,
             NSLocalizedRecoverySuggestionErrorKey:LocationErrors.reverseGeocodingFailed.rawValue]))
                return
            }
    ...
    private func check(status: CLAuthorizationStatus?) {
        ...
        case .denied:
            let deniedError = NSError(domain: self.classForCoder.description(),
                code:Int(CLAuthorizationStatus.denied.rawValue),
        ...
        case .restricted:
            ...
            if reverseGeocoding {
                didComplete(location: nil,error: NSError(domain: self.classForCoder.description(),
                    code:Int(CLAuthorizationStatus.restricted.rawValue),userInfo: nil))
            } else {
                didComplete(location: nil,error: NSError(domain: self.classForCoder.description(),
                    code:Int(CLAuthorizationStatus.restricted.rawValue), userInfo: nil))
            }
        ...
        @unknown default:
            didComplete(location: nil,error: NSError(domain: self.classForCoder.description(),
                code:Int(CLAuthorizationStatus.denied.rawValue),
                userInfo:[NSLocalizedDescriptionKey:LocationErrors.unknown.rawValue,
...
// MARK: - this add polyline to mapView and get expected Time Travel and distance

extension LocationManager {
    
    func getDirection(_ location: CLLocation,completion:@escaping ((String?,MKRoute,MKCoordinateRegion,NSNumber)->())){
        
        let request = MKDirections.Request()
        
        let sourcePlacemark = MKPlacemark(coordinate: CLLocationCoordinate2D(latitude: location.coordinate.latitude, longitude: location.coordinate.longitude))
        
        request.source = MKMapItem(placemark: sourcePlacemark)
        
        let destinationPlacemark = MKPlacemark(coordinate: CLLocationCoordinate2D(latitude: 44.9729, longitude: -93.2833))
        
        let newLatitude = (location.coordinate.latitude + 44.9729)/2
        let newLongitude = (location.coordinate.longitude + -93.2833)/2
        ...
        directions.calculate { response, error in
            ...
            if let route = response?.routes.first {
                ...
                let date = Date().addingTimeInterval(route.expectedTravelTime + 900).timeIntervalSince1970 as NSNumber
                
                let info: String = "\(Int(route.expectedTravelTime/60)) min | \(self.distanceConverter(distance: route.distance))"
                
                completion(info,route,region, date)
            }
        }
    }
